...
export default function ReviewList({ reviews, total, page, signedIn, buildHref, scope }: {
    ...
    return <section id="student-reviews" className="scroll-mt-24 my-10" aria-labelledby="student-review-title">
        ...
        {!reviews.length ? <div className="coffee-card"><p>{total ? 'No reviews on this page.' : 'No student reviews yet.'}</p>{total > 0 && <a href={buildHref(1)} className="underline">First page</a>}</div> : reviews.map(review => <ReviewItem key={review.id} review={review} signedIn={signedIn} />)}
